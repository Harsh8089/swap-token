...
function Swap() {
    ...
    const [phantomLoader, setPhantomLoader] = useState<boolean>(false);
    ...
    const swapToken = async() => {
        if (!inAmount || Number(inAmount) === 0) {
            toast.error('Solana value must be greater than 0');
            return;
        }
        setPhantomLoader(true);
        if(publicKey && signTransaction ) {
            try {
                ...
                setPhantomLoader(false);
                console.log("Error performing swap:", error);
                toast.error("Please check console log");
            }
        }
        else {
            setPhantomLoader(false);
    ...
    return (
        <div className={`relative w-[45vw] flex justify-center items-center mt-12 mx-auto z-0`}>
            {
                phantomLoader ? 
                (
                    <div className="flex flex-col items-center gap-6">
                        <Spinner />
                        <p className="text-purple-400 text-xl tracking-wider">Processing your Transaction ...</p>
                    </div>
                ):
                (
                    <>
                        <div className={`${tokenOptType != "" ? 'blur-md overflow-hidden' : ''} w-full flex flex-col rounded-2xl shadow-xl p-6`}>
